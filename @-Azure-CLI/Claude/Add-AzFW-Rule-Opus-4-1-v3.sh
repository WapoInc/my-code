...
#=================================================================================================================================================
# Add HTTP rule to allow traffic from 192.168.2.0/24 to 10.40.0.0/22
#=================================================================================================================================================
echo "Checking if HTTP rule collection exists..."
if ! az network firewall policy rule-collection-group collection show --resource-group $RESOURCE_GROUP --policy-name AzFW-Policy --rule-collection-group-name NetworkRuleCollectionGroup --name AllowHTTPCollection &>/dev/null; then
    echo "Creating HTTP rule collection..."
    az network firewall policy rule-collection-group collection add-filter-collection \
        --resource-group $RESOURCE_GROUP \
        --policy-name AzFW-Policy \
        --rule-collection-group-name NetworkRuleCollectionGroup \
        --name AllowHTTPCollection \
        --collection-priority 130 \
        --action Allow \
        --rule-name AllowHTTPFromOnPrem \
        --rule-type NetworkRule \
        --source-addresses "192.168.2.0/24" \
        --destination-addresses "10.40.0.0/22" \
    ...
    echo "HTTP rule collection already exists, skipping..."
fi
...
echo "Deployment completed with RDP, SSH, HTTPS, and HTTP network rules!"