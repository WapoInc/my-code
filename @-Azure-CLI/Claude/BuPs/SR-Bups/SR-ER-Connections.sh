...
GATEWAY_RESOURCE_GROUP="Claude-rg4"
GATEWAY_NAME="zan-ergw-azure-hub"
...
CONNECTION_NAME="zan-er-connection-hub"
CONNECTION_RESOURCE_GROUP="Claude-rg4"  # Creating connection in same RG as gateway
...
if [ "$GATEWAY_STATE" != "Succeeded" ]; then
    echo "ERROR: Gateway $GATEWAY_NAME is not ready. Current state: $GATEWAY_STATE"
    echo "Please wait for the gateway to be fully provisioned before creating the connection."
    exit 1
fi
echo "✓ Gateway $GATEWAY_NAME is ready (State: $GATEWAY_STATE)"
...
EXISTING_CONNECTION=$(az network vpn-connection show \
    --resource-group $CONNECTION_RESOURCE_GROUP \
...
echo "================================================"
echo "Gateway: $GATEWAY_NAME (RG: $GATEWAY_RESOURCE_GROUP)"
echo "Circuit: $CIRCUIT_NAME (RG: $CIRCUIT_RESOURCE_GROUP)"
echo "Connection: $CONNECTION_NAME (RG: $CONNECTION_RESOURCE_GROUP)"
echo ""
echo "Next Steps:"
...
echo "    --resource-group $CONNECTION_RESOURCE_GROUP \\"
