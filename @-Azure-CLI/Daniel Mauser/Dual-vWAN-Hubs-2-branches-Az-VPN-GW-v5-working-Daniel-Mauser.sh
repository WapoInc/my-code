...
# Boot diagnostics removed - az vm boot-diagnostics enable conflicts with customScript extension on Python 3.13 CLI builds
...
# *** FIX: Wait for vWAN VPN gateway connections to succeed before creating local gateways ***
echo Waiting for site-branch1-conn to succeed...
prState=''
while [[ $prState != 'Succeeded' ]];
do
...
echo Waiting for site-branch2-conn to succeed...
prState=''
while [[ $prState != 'Succeeded' ]];
do
    prState=$(az network vpn-gateway connection show \
