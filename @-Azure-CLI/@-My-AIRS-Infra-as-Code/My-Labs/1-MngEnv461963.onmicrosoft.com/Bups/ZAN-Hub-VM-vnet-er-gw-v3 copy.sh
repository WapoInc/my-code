#!/bin/bash
# ============================================================
# Deploy SA-North-HUB - VNet + Subnets + ER GW + VM
# South Africa North  |  v3 - Parallel Deployment
# ============================================================
# Resources:
...
#   - AzureFirewallManagementSubnet : 10.10.0.128/26
#   - AzureBastionSubnet            : 10.10.0.192/26
...
#   - DnsResolverInboundSubnet      : 10.10.2.64/27 (delegated to Microsoft.Network/dnsResolvers)
#   - DnsResolverOutboundSubnet     : 10.10.2.96/27 (delegated to Microsoft.Network/dnsResolvers)
...
#   Phase 2 : VNet || Public IP || NSG       (parallel)
#   Phase 3 : ALL subnets                    (sequential after VNet)
#   Phase 4 : ER Gateway (background)         (after ALL subnets)
#   Phase 5 : VM NIC                          (after subnets, GW in background)
#   Phase 6 : Ubuntu VM                       (after NIC, GW in background)
...
GATEWAY_SUBNET_PREFIX="10.10.0.0/26"
FIREWALL_SUBNET_PREFIX="10.10.0.64/26"
FIREWALL_MGMT_SUBNET_PREFIX="10.10.0.128/26"
BASTION_SUBNET_PREFIX="10.10.0.192/26"
SUBNET1_NAME="SubNet-1"
SUBNET1_PREFIX="10.10.1.0/24"
ROUTESERVER_SUBNET_PREFIX="10.10.2.0/26"
DNS_INBOUND_SUBNET_PREFIX="10.10.2.64/27"
DNS_OUTBOUND_SUBNET_PREFIX="10.10.2.96/27"
APPGW_SUBNET_PREFIX="10.10.3.0/24"
...
deploy_firewall_subnet() {
  if az network vnet subnet show --resource-group "$RESOURCE_GROUP" --vnet-name "$VNET_NAME" --name "AzureFirewallSubnet" &>/dev/null; then
    echo "[FWSUB] AzureFirewallSubnet found in VNet '$VNET_NAME'."
    echo "[SKIP]     Skipping creation."
  else
    echo "[FWSUB] Creating AzureFirewallSubnet ($FIREWALL_SUBNET_PREFIX)..."
    az network vnet subnet create \
      --resource-group "$RESOURCE_GROUP" \
      --vnet-name "$VNET_NAME" \
...
deploy_firewall_mgmt_subnet() {
  if az network vnet subnet show --resource-group "$RESOURCE_GROUP" --vnet-name "$VNET_NAME" --name "AzureFirewallManagementSubnet" &>/dev/null; then
    echo "[FWMSUB] AzureFirewallManagementSubnet found in VNet '$VNET_NAME'."
    echo "[SKIP]     Skipping creation."
  else
    echo "[FWMSUB] Creating AzureFirewallManagementSubnet ($FIREWALL_MGMT_SUBNET_PREFIX)..."
    az network vnet subnet create \
      --resource-group "$RESOURCE_GROUP" \
      --vnet-name "$VNET_NAME" \
      --name "AzureFirewallManagementSubnet" \
      --address-prefix "$FIREWALL_MGMT_SUBNET_PREFIX" \
      --output none
    echo "[FWMSUB] Done."
  fi
}

# DNS Private Resolver inbound endpoint subnet (must be delegated to Microsoft.Network/dnsResolvers).
deploy_dns_inbound_subnet() {
  if az network vnet subnet show --resource-group "$RESOURCE_GROUP" --vnet-name "$VNET_NAME" --name "DnsResolverInboundSubnet" &>/dev/null; then
    echo "[DNSIN] DnsResolverInboundSubnet found in VNet '$VNET_NAME'."
    echo "[SKIP]     Skipping creation."
  else
    echo "[DNSIN] Creating DnsResolverInboundSubnet ($DNS_INBOUND_SUBNET_PREFIX)..."
    az network vnet subnet create \
      --resource-group "$RESOURCE_GROUP" \
      --vnet-name "$VNET_NAME" \
      --name "DnsResolverInboundSubnet" \
      --address-prefix "$DNS_INBOUND_SUBNET_PREFIX" \
...
# DNS Private Resolver outbound endpoint subnet (must be delegated to Microsoft.Network/dnsResolvers).
deploy_dns_outbound_subnet() {
  if az network vnet subnet show --resource-group "$RESOURCE_GROUP" --vnet-name "$VNET_NAME" --name "DnsResolverOutboundSubnet" &>/dev/null; then
    echo "[DNSOUT] DnsResolverOutboundSubnet found in VNet '$VNET_NAME'."
    echo "[SKIP]     Skipping creation."
  else
    echo "[DNSOUT] Creating DnsResolverOutboundSubnet ($DNS_OUTBOUND_SUBNET_PREFIX)..."
    az network vnet subnet create \
      --resource-group "$RESOURCE_GROUP" \
      --vnet-name "$VNET_NAME" \
      --name "DnsResolverOutboundSubnet" \
      --address-prefix "$DNS_OUTBOUND_SUBNET_PREFIX" \
      --delegations "Microsoft.Network/dnsResolvers" \
      --output none
    echo "[DNSOUT] Done."
  fi
...
deploy_appgw_subnet() {
  if az network vnet subnet show --resource-group "$RESOURCE_GROUP" --vnet-name "$VNET_NAME" --name "AppGatewaySubnet" &>/dev/null; then
    echo "[AGWSUB] AppGatewaySubnet found in VNet '$VNET_NAME'."
    echo "[SKIP]     Skipping creation."
  else
    echo "[AGWSUB] Creating AppGatewaySubnet ($APPGW_SUBNET_PREFIX)..."
    az network vnet subnet create \
      --resource-group "$RESOURCE_GROUP" \
      --vnet-name "$VNET_NAME" \
...
# --------------------------------------------------
echo ""
echo "==> PHASE 3: ALL subnets (sequential - must all complete before ER GW starts)"
deploy_gateway_subnet
deploy_firewall_subnet
deploy_firewall_mgmt_subnet
deploy_bastion_subnet
...
# --------------------------------------------------
echo ""
echo "==> PHASE 4: ER Gateway launched in BACKGROUND (after ALL subnets)"
echo "            (ER GW runs in background while the VM is built)"
deploy_er_gateway &
PID_GW=$!

# --------------------------------------------------
echo ""
echo "==> PHASE 5: VM NIC (after subnets, ER GW still running in background)"
deploy_vm_nic
