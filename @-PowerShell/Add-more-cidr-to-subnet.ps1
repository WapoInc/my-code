...
New-AzResourceGroup -Name 'SubnetDemoRG' -Location 'SouthAfricaNorth'
...
$vnet='ER-LTSA-RG-vnet'
$SubnetDemoRG='er-ltsa-rg'


## Example 3: Remove a prefix from a multi-prefix subnet.
# Get the existing virtual network
$vnet = Get-AzVirtualNetwork -ResourceGroupName $SubnetDemoRG -Name $vnet
# View existing subnets
$vnet.Subnets
...
$subnet = @{
    virtualNetwork          = $vnet
    Name                     = 'GatewaySubnet'
    AddressPrefix            = '10.10.0.0/24'  # Remove the second prefix'
}
set-AzVirtualNetworkSubnetConfig @subnet
