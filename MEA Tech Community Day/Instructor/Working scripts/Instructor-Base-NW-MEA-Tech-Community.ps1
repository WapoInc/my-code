# Requires Az PowerShell module 112233
...
$gwJobs += Start-Job -Name "onprem-gateway-deployment" -ScriptBlock {
  param($rg, $loc, $vnetName, $pipName, $subscriptionId, $tenantId)
  ...
  Set-AzContext -SubscriptionId $subscriptionId -TenantId $tenantId | Out-Null
    ...
    New-AzVirtualNetworkGateway `
      ...
      -VpnType RouteBased `
      -GatewaySku VpnGw1
} -ArgumentList $ResourceGroup, $Location, $OnpremVnet, "onprem-gateway-pip", $SubscriptionId, $TenantId

$gwJobs += Start-Job -Name "azure-gateway-deployment" -ScriptBlock {
    param($rg, $loc, $vnetName, $pipName, $subscriptionId, $tenantId)
  ...
  $ErrorActionPreference = "Stop"
    ...
    New-AzVirtualNetworkGateway `
      ...
      -VpnType RouteBased `
      -GatewaySku VpnGw1
    } -ArgumentList $ResourceGroup, $Location, $AzureVnet, "azure-gateway-pip", $SubscriptionId, $TenantId
...
$failedGwJobs = @($gwJobs | Where-Object State -ne "Completed")
if ($failedGwJobs.Count -gt 0) {
  Write-Host "=============================================================================" -ForegroundColor Red
  Write-Host "VPN Gateway deployment failed. Azure error details:" -ForegroundColor Red
  Write-Host "=============================================================================" -ForegroundColor Red

  $failedGwJobs | Receive-Job -Keep
  $failedGwJobNames = ($failedGwJobs.Name -join ", ")
  $gwJobs | Remove-Job -Force
  throw "VPN Gateway deployment job(s) failed: $failedGwJobNames. Connection creation was skipped."
}

$gwJobs | Receive-Job | Out-Null
...
$onpremGateway = Get-AzVirtualNetworkGateway -ResourceGroupName $ResourceGroup -Name "onprem-gateway" -ErrorAction Stop
$azureGateway = Get-AzVirtualNetworkGateway -ResourceGroupName $ResourceGroup -Name "azure-gateway" -ErrorAction Stop
