...
$onpremVnetObj = New-AzVirtualNetwork `
  -ResourceGroupName $ResourceGroup `
  -Location $Location `
  -Name $OnpremVnet `
  -AddressPrefix @($OnpremVnetPrefix, $OnpremVnetPrefix2) `
  -Subnet @($onpremSubnet, $onpremGwSubnet, $onpremSubnet4)
...
Write-Host "✓ Network rule created: Allow traffic from 192.168.1.0/24 to 10.70.1.0/24" -ForegroundColor Green
